...
const Comment = ({data , t , user , handleSubmit , setContent , replay}:any) => {
    ...
    <div className="py-8 border-b relative">
      ...
      {user && !replay && <FaReply onClick={()=>setshow(true)} className="absolute ltr:right-4 rtl:left-4 bottom-4 cursor-pointer"/>}
      
        {show && (
                <form className="grid  lg:grid-cols-2 gap-5 mt-10 relative ">
        <Heading>
ریپلای به        {data.user?.name} 
        </Heading>
                ...
                <span className="col-span-full">
                  <Button variant="primary" className="w-full" onClick={()=>handleSubmit("reply" , data._id)} >
                  {t("send")}
                  </Button>
                  <button
    ...
    {data?.replays?.map((item)=><Comment replay={data} data={item} t={t} user={user} handleSubmit={handleSubmit} setContent={setContent}/>)}
    </div>
    </>
    )}
