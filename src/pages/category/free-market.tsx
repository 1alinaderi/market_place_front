...
const FreeMarketCategoryPage = () => {
  ...
  async function fetchData(_id ?: string) {
    ...
    await httpReauest(
      'GET',
      `/prouduct/free?page=${page}&limit=${limit}&category=${_id ? _id : selected._id }&sort=-createdAt`,
      {},
      {}
    ).then(({ data }) => {
    ...
    <div>
      ...
      <div className="flex gap-2 flex-wrap items-start justify-center text-black px-4 py-5">
         {selctedSub?.map((item: any) => (
            <Link href={`/free-market/${selected?.url}/${item.url}`}>
...
export default FreeMarketCategoryPage;
