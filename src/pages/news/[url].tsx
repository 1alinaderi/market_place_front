...
export default function NewsDetailPage(props: any) {
  ...
  async function getData() {
    ...
    await httpReauest(
      'GET',
      '/news?page=1&limit=12&category=' + data?.category,
      {},
      {}
    ).then((res) => {
          ...
          <span className="col-span-3 lg:col-span-2">
            <div>
              <img
                ...
                className="w-full rounded-[15px] max-h-[480px] object-cover"
              />
            </div>
            <div className="mb-4 lg:mb-8">
              <h2 className="lg:text-3xl text-xl font-bold mt-[2rem]">
                {data?.name}
              </h2>
            ...
            <div
              dir="ltr"
              className="mt-10 border-t border-b border-[#73818D57] py-3"
            >
          ...
          <span className="hidden lg:inline">
            <div className="border rounded-[15px] p-5 flex flex-col gap-3 bg-[#f7f7f7]">
              <h2 className="text-[20px] flex items-center gap-2">
                <FaSquare size={16} className="mt-1" color="#F37324" />{' '}
                {t('realated-news')}
              </h2>
              {blogs?.map((item) => (
                <BlogSmallCard data={item} />
              ))}
            </div>
          </span>
        </div>
